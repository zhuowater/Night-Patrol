...
> 当前判断：`v0.2.10`「可学习失败」切片已进入实现阶段：本局 SOC 复盘、事件路线构筑补偿、短视口信息层级、地图 CTA 和构筑诊断应作为下一轮真实闭门试玩基线；真实反馈表切到 `docs/playtest/v0.2.10-feedback-form.md`。

## 2026-05-03 v0.2.10 实现/试玩材料更新

v0.2.10 计划与闭门试玩材料已补齐：

- 开发计划：`docs/plans/2026-05-03-v0.2.10-learnable-run-recap.md`
- 闭门试玩说明：`docs/playtest/v0.2.10-closed-playtest-instructions.md`
- 反馈表：`docs/playtest/v0.2.10-feedback-form.md`
- 反馈汇总模板：`docs/playtest/v0.2.10-feedback-summary-template.md`

下一轮真实试玩的提问重心从 v0.2.9 的“第一局是否知道该做什么”升级为：

1. 第一局结束后，玩家能否复述为什么赢/输；
2. 复盘页哪条信息最有用，哪条没用或不准；
3. 玩家是否知道下一局该改路线、构筑还是战斗打法；
4. 事件路线让玩家觉得赚了、亏了，还是只是读故事；
5. Boss 入场前玩家是否知道血线危险。

---

## 2026-05-03 v0.2.10 计划更新

基于最新完整试玩体验报告：

- 完整体验报告：`reports/playtest-v0.2.9-demo-complete-experience-report-2026-05-03.md`
- 5 人模拟详细报告：`reports/playtest-v0.2.9-demo-5-independent-players-post-hotfix-2026-05-03.md`
- 下一版本计划：`docs/plans/2026-05-03-v0.2.10-learnable-run-recap.md`

下一阶段判断：

1. **v0.2.10 不扩内容**：不新增章节、敌人、卡牌大包或 Boss 大改；先解决“玩家死后不知道为什么”的反馈污染问题。
2. **最高优先级是本局 SOC 复盘页**：记录路线、Boss 入场血量、关键战斗、事件/奖励、工具、死亡/胜利原因推断和下一局建议。
3. **事件路线需要构筑补偿**：安全玩家会被事件文本吸引，不能让多事件路线天然弱于战斗/商店路线。
4. **短视口进入第二轮信息层级**：hotfix 已缓解文字重叠，下一步是压缩/折叠专家态势与建议框，强化关键 CTA。
5. **真实闭门试玩应在 v0.2.10 后再拉**：让反馈从“Boss 太难/我不知道为什么死”升级为“复盘是否准确、下一局建议是否有用”。
6. **旧工程主线显式后置**：release workflow 统一、资源预算第二轮、Phaser/media warning、内容扩展进入 v0.2.11/v0.3.0 候选，不混入 v0.2.10。
