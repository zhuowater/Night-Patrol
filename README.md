...
当前版本是 `v0.2.10-demo` 闭门试玩候选：可以从标题页进入攻击路径，经历普通战、高危入侵、异常事件、情报市场、维护窗口，最终阻断核心域控前的勒索核心。本版在 v0.2.9 分发/资源治理基础上，补齐本局 SOC 复盘、失败原因推断、事件路线构筑补偿、地图 CTA 与构筑缺口提示，让失败更可解释、可学习。
...
- 最新试玩包：`v0.2.10-demo`
- 下载页面：https://github.com/zhuowater/Night-Patrol/releases/tag/v0.2.10-demo
...
| 系统 | 推荐下载 | 备用 | 备注 |
|---|---|---|---|
| Windows x64 | `Night-Patrol-SOC-0.2.10-win-x64.exe` | `Night-Patrol-SOC-0.2.10-win-x64.zip` | 如果 SmartScreen 拦截，点“更多信息”→“仍要运行”；或改用 zip 版。 |
| macOS Apple Silicon | `Night-Patrol-SOC-0.2.10-mac-arm64.dmg` | `Night-Patrol-SOC-0.2.10-mac-arm64.zip` | 当前是 Apple Silicon 包；demo 阶段未公证，首次打开可能需要右键“打开”。 |
| Linux x64 | `Night-Patrol-SOC-0.2.10-linux-x86_64.AppImage` | `Night-Patrol-SOC-0.2.10-linux-x64.zip` | AppImage 可能需要先执行 `chmod +x Night-Patrol*.AppImage`。 |
...
```bash
git tag v0.2.10-demo
git push origin v0.2.10-demo
```
...
- `docs/RELEASE_NOTES_v0.2.10-demo.md`：v0.2.10 Release Notes。
